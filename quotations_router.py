...
def _schema(db):
    stmts = [
        """CREATE TABLE IF NOT EXISTS quotations (
            id SERIAL PRIMARY KEY, ref VARCHAR NOT NULL,
            mode VARCHAR DEFAULT 'Ocean', client VARCHAR, client_email VARCHAR,
            carrier VARCHAR, pol VARCHAR, pod VARCHAR, incoterm VARCHAR,
            validity_date VARCHAR, transit_days VARCHAR,
            status VARCHAR DEFAULT 'Pending', note TEXT,
        ...
        "ALTER TABLE quotations ADD COLUMN IF NOT EXISTS incoterm VARCHAR",
        ...
        """CREATE TABLE IF NOT EXISTS quotation_charges (
            ...
            name VARCHAR NOT NULL, amount VARCHAR,
            currency VARCHAR DEFAULT 'USD', unit VARCHAR
        )""",
...
def _charges(db, qid):
    rows = db.execute(text(
        "SELECT id,name,amount,currency,unit FROM quotation_charges WHERE quotation_id=:q ORDER BY id"
    ), {"q": qid}).fetchall()
    return [{"id":r[0],"name":r[1],"amount":r[2],"currency":r[3],"unit":r[4]} for r in rows]

def _containers(db, qid):
    rows = db.execute(text(
        "SELECT id,qty,ctype FROM quotation_containers WHERE quotation_id=:q ORDER BY id"
    ), {"q": qid}).fetchall()
    return [{"id":r[0],"qty":r[1],"ctype":r[2]} for r in rows]

def _ser(db, r):
    return {
        "id":r[0],"ref":r[1],"mode":r[2],"client":r[3],"clientemail":r[4],
        "carrier":r[5],"pol":r[6],"pod":r[7],"incoterm":r[8],
        "validitydate":r[9],"transitdays":r[10],"status":r[11],"note":r[12],"currency":r[13],
...
def _write_children(db, qid, body):
    ...
    for ch in (body.get("charges") or []):
        if isinstance(ch, dict) and (ch.get("name") or "").strip():
            db.execute(text(
                "INSERT INTO quotation_charges (quotation_id,name,amount,currency,unit) VALUES (:q,:n,:a,:c,:u)"
    ...
    for ct in (body.get("containers") or []):
        if isinstance(ct, dict) and ct.get("qty") and ct.get("ctype"):
            try:
                db.execute(text(
                    "INSERT INTO quotation_containers (quotation_id,qty,ctype) VALUES (:q,:qty,:ct)"
...
def list_q(q:str="", status:str="", mode:str="", db:Session=Depends(get_db), _=Depends(get_current_user)):
    ...
    rows = db.execute(text(
        f"SELECT id,ref,mode,client,client_email,carrier,pol,pod,incoterm,validity_date,transit_days,status,note,currency FROM quotations {where} ORDER BY id DESC"
    ), params).fetchall()
...
def get_q(qid:int, db:Session=Depends(get_db), _=Depends(get_current_user)):
    _schema(db)
    r = db.execute(text(
        "SELECT id,ref,mode,client,client_email,carrier,pol,pod,incoterm,validity_date,transit_days,status,note,currency FROM quotations WHERE id=:id"
    ), {"id":qid}).fetchone()
    ...
    try:
        row = db.execute(text("""
            INSERT INTO quotations
                (ref, mode, client, client_email, carrier, pol, pod, incoterm,
                 validity_date, transit_days, status, note, currency, created_at, updated_at)
            VALUES
                (:ref, :mode, :client, :ce, :carrier, :pol, :pod, :inc,
        ...
        db.rollback(); raise HTTPException(500, f"{type(e).__name__}: {e}")
    ...
    try:
        db.execute(text("""
            UPDATE quotations SET
                mode=:mode, client=:client, client_email=:ce, carrier=:carrier,
                pol=:pol, pod=:pod, incoterm=:inc, validity_date=:vd,
...
def quotation_pdf(qid:int, db:Session=Depends(get_db), _=Depends(get_current_user)):
    ...
    try:
        ...
        if s['containers']:
            ...
            story.append(ct)
        story += [Spacer(1,4*mm), Paragraph('CHARGES', hs)]
