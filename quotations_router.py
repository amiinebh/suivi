...
def _charges(db, qid):
    r = db.execute(text("SELECT id,name,amount,currency,unit FROM quotation_charges WHERE quotation_id=:q ORDER BY id"), {"q":qid}).fetchall()
    return [{"id":x[0],"name":x[1],"amount":x[2],"currency":x[3],"unit":x[4]} for x in r]
...
def _serialize(db, row):
    return {
        "id":row[0], "ref":row[1], "mode":row[2], "client":row[3], "clientemail":row[4],
        "carrier":row[5], "pol":row[6], "pod":row[7], "incoterm":row[8],
        "validitydate":row[9], "transitdays":row[10], "status":row[11],
        "note":row[12], "currency":row[13],
        "charges":_charges(db,row[0]), "containers":_containers(db,row[0]),
    }
    ...
    for ch in (body.get("charges") or []):
        if isinstance(ch,dict) and (ch.get("name") or "").strip():
            db.execute(text("INSERT INTO quotation_charges (quotation_id,name,amount,currency,unit) VALUES (:q,:n,:a,:c,:u)"),
                {"q":qid,"n":ch["name"].strip(),"a":ch.get("amount"),"c":ch.get("currency","USD"),"u":ch.get("unit","Per BL")})
    for ct in (body.get("containers") or []):
        if isinstance(ct,dict) and ct.get("qty") and ct.get("ctype"):
            try: db.execute(text("INSERT INTO quotation_containers (quotation_id,qty,ctype) VALUES (:q,:qty,:ct)"), {"q":qid,"qty":int(ct["qty"]),"ct":ct["ctype"]})
            except: pass
    db.commit()
...
async def create_q(request:Request, db:Session=Depends(get_db), _=Depends(get_current_user)):
    ...
    try:
        row=db.execute(text("""INSERT INTO quotations (ref,mode,client,client_email,carrier,pol,pod,incoterm,validity_date,transit_days,status,note,currency,created_at,updated_at)
...
def quotation_pdf(qid:int, db:Session=Depends(get_db), _=Depends(get_current_user)):
    ...
    title_style=ParagraphStyle('t',parent=styles['Heading1'],alignment=TA_CENTER,textColor=colors.HexColor('#21808D'),fontSize=18)
    head_style=ParagraphStyle('h',parent=styles['Heading2'],textColor=colors.HexColor('#21808D'),fontSize=11)
    story=[Paragraph('FREIGHT QUOTATION',title_style),Spacer(1,4*mm),
           Paragraph(f'<b>Ref:</b> {s["ref"]}  &nbsp;&nbsp; <b>Valid Until:</b> {s["validitydate"] or "N/A"}  &nbsp;&nbsp; <b>Transit:</b> {s["transitdays"] or "N/A"} days',styles['Normal']),
           Spacer(1,4*mm),Paragraph('SHIPMENT DETAILS',head_style)]
    ...
    story.append(info)
    if s['containers']:
        story+=[Spacer(1,4*mm),Paragraph('CONTAINERS',head_style)]
        ...
        story.append(ct)
    story+=[Spacer(1,4*mm),Paragraph('CHARGES',head_style)]
    ...
    if s['note']: story+=[Spacer(1,4*mm),Paragraph('NOTES',head_style),Paragraph((s['note']or'').replace('\n','<br/>'),styles['Normal'])]
