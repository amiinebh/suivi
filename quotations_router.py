...
async def create_quotation(request: Request, db: Session = Depends(get_db), current=Depends(get_current_user)):
    ...
    try:
        q = Quotation(ref=ref, mode=body.get('mode') or 'Ocean', client=body.get('client'), client_email=body.get('clientemail'), carrier=body.get('carrier'), pol=body.get('pol'), pod=body.get('pod'), booking_no=body.get('bookingno'), incoterm=body.get('incoterm'), validity_date=body.get('validitydate'), status=body.get('status') or 'Pending', note=body.get('note'), currency=body.get('currency') or 'USD', created_at=now, updated_at=now)
        db.add(q)
        db.flush()
        for c in (body.get('charges') or []):
            if c.get('name'):
                db.add(QuotationCharge(quotation_id=q.id, name=str(c.get('name')), amount=None if c.get('amount') in (None, '') else str(c.get('amount')), currency=str(c.get('currency') or 'USD'), unit=str(c.get('unit') or ''), note=str(c.get('note') or '')))
        for ct in (body.get('containers') or []):
            qty = ct.get('qty'); ctype = ct.get('ctype')
            if qty and ctype:
...
async def update_quotation(qid: int, request: Request, db: Session = Depends(get_db), current=Depends(get_current_user)):
    ...
    body = await request.json()
    try:
        mapping = {'mode':'mode','client':'client','clientemail':'client_email','carrier':'carrier','pol':'pol','pod':'pod','bookingno':'booking_no','incoterm':'incoterm','validitydate':'validity_date','status':'status','note':'note','currency':'currency'}
        ...
        if 'charges' in body:
            ...
            for c in (body.get('charges') or []):
                if c.get('name'):
                    db.add(QuotationCharge(quotation_id=qid, name=str(c.get('name')), amount=None if c.get('amount') in (None, '') else str(c.get('amount')), currency=str(c.get('currency') or 'USD'), unit=str(c.get('unit') or ''), note=str(c.get('note') or '')))
